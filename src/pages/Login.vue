        ...
        >
          ...
          <q-btn-group push flat unelevated class="full-width row q-mt-md">
            <q-btn
              ...
              color="positive"
              @click="!codeSent ? sendCode() : (checkCode(), onSubmit())"
            />
          </q-btn-group>
        </div>
        </q-form>
...
export default {
    ...
    mounted(){
      ...
      console.log("PIAZZETTA store", this.$store.getters.getLogin)
      console.log("PIAZZETTA store", this.$store.getters.getLogin == [])
      if(this.$store.getters.getLogin.user){
        this.$router.push("/hotmilk")
      }else{
        this.$store.dispatch("setarDados", {key: "setEstadoInicial", value: true,});
      }
    },
    methods: {
    async submit() {
    ...
    async checkCode() {
      ...
      Loading.hide();
      if (response.status == 200) {
        console.log("PIAZZETTA response.data", response.data)
